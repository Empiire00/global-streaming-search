<script lang="ts">
  ...
  function unique<Type extends any>(a: Type[]): Type[] {
    return [...new Map(a.map((item) => [item["id"], item])).values()];
  }
</script>
    ...
    <div class="box">
      ...
      <div>
        <div style="display:flex; justify-content: right;">
          <label>
            <input type="checkbox" bind:checked={show_all_countries} />
            Show all countries
          </label>
        </div>
        {#each titles.offers.sort(compare_offers) as country}
          {#key show_all_countries}
            {#if country.offers.length > 0 || show_all_countries}
              <div class="offer">
                  ...
                  <div style="display: inline-block;">
                    <span style="text-align: center; display: inline-block;">
                      <a href={offer["url"]}
                        ><img
                          src={offer["icon_uri"]}
                          alt="Icon for {offer.clear_name}"
                          id={offer.short_name}
                        /></a
                      >
                      <label for={offer.short_name}>{offer.clear_name}</label>
                      {#if offer.seasons != undefined}
                        <label for={offer.short_name}
                          >{offer.seasons} / {titles["number_seasons"]} Seasons</label
                        >
                      {/if}
                      <label for={offer.short_name}>
                        {offer.monetization_types.reduce(
                          (a, b) => a + ", " + b
                        )}
                      </label>
                    </span>
